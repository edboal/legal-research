...
import { Editor as Editor54 } from "slate";
...
var HistoryEditor = {
  ...
  isHistoryEditor(value) {
    return History.isHistory(value.history) && Editor54.isEditor(value);
  },
...
import { Editor as Editor55, Operation as Operation2, Path as Path3, Transforms as Transforms5 } from "slate";
var withHistory = (editor) => {
  ...
  e.redo = () => {
    ...
    if (redos.length > 0) {
      ...
      if (batch.selectionBefore) {
        Transforms5.setSelection(e, batch.selectionBefore);
      }
      HistoryEditor.withoutSaving(e, () => {
        Editor55.withoutNormalizing(e, () => {
  ...
  e.undo = () => {
    ...
    if (undos.length > 0) {
      ...
      HistoryEditor.withoutSaving(e, () => {
        Editor55.withoutNormalizing(e, () => {
          const inverseOps = batch.operations.map(Operation2.inverse).reverse();
...
var shouldMerge = (op, prev) => {
  if (prev && op.type === "insert_text" && prev.type === "insert_text" && op.offset === prev.offset + prev.text.length && Path3.equals(op.path, prev.path)) {
    return true;
  }
  if (prev && op.type === "remove_text" && prev.type === "remove_text" && op.offset + op.text.length === prev.offset && Path3.equals(op.path, prev.path)) {
    return true;
  }
  return false;
};
...
var collapseSelection = (editor, options) => {
  Transforms6.collapse(editor, options);
};

// src/interfaces/transforms/deselect.ts
import { Transforms as Transforms7 } from "slate";
var deselect = (editor) => {
  Transforms7.deselect(editor);
};

// src/interfaces/transforms/insertFragment.ts
import { Transforms as Transforms8 } from "slate";
var insertFragment = (editor, fragment, options) => {
  Transforms8.insertFragment(editor, fragment, options);
};

// src/interfaces/transforms/insertNodes.ts
import { Path as Path4, Transforms as Transforms9, removeNodes as removeNodes2, withoutNormalizing as withoutNormalizing2 } from "slate";
  ...
  withoutNormalizing2(editor, () => {
    ...
    if (nextBlock) {
      ...
      if (at) {
        ...
        if (blockEntry) {
          options.at = Path4.next(blockEntry[1]);
        }
      }
    }
    Transforms9.insertNodes(editor, nodes, options);
  });
};
...
var wrapNodes = (editor, element, options) => {
  unhangRange(editor, options == null ? void 0 : options.at, options);
  Transforms19.wrapNodes(editor, element, options);
};
...
var addRangeMarks = (editor, props, {
  ...
  if (at) {
    ...
    const match2 = (node, path) => {
      ...
      const parentEntry = Editor56.parent(editor, path);
    ...
    if (isExpandedRange || markAcceptingVoidSelected) {
      Transforms20.setNodes(editor, props, {
        at,
        match: match2,
        split: true,
        voids: true
...
import { Path as Path6, Range as Range7 } from "slate";
var unhangCharacterRange = (editor, at) => {
  let [start, end] = Range7.edges(at);
  if (!Path6.equals(start.path, end.path)) {
    if (end.offset === 0) {
