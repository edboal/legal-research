...
__export(src_exports, {
  ...
  getFirstNodeText: () => getFirstNodeText,
  getFragmentProp: () => getFragmentProp,
  ...
  removeEmptyPreviousBlock: () => removeEmptyPreviousBlock,
  ...
  setBlockAboveNode: () => setBlockAboveNode,
  setBlockAboveTexts: () => setBlockAboveTexts,
  setBlockNodes: () => setBlockNodes,
  setMarks: () => setMarks,
  toggleMark: () => toggleMark,
  toggleWrapNodes: () => toggleWrapNodes,
  unwrapStructuralNodes: () => unwrapStructuralNodes,
...
var getBlocks = (editor, options) => {
  return [
    ...(0, import_slate5.getNodeEntries)(editor, __spreadProps(__spreadValues({}, options), {
      block: true
    }))
  ];
};
...
var getChildren = (nodeEntry) => {
  ...
  if ((0, import_slate6.isAncestor)(node)) {
    const { children } = node;
...
var getEdgeBlocksAbove = (editor, _a = {}) => {
  ...
  if (!at) return null;
  const [start, end] = (0, import_slate7.getEdgePoints)(editor, at != null ? at : editor.selection);
...
var getFirstNodeText = (root, options) => {
  const texts = (0, import_slate8.getNodeTexts)(root, options);
...
function getFragmentProp(fragment, { key, defaultValue, getProp, mode = "block" } = {}) {
  ...
  for (const node of fragment) {
    ...
    if (mode === "text" || mode === "all") {
      const textEntries = Array.from((0, import_slate9.getNodeTexts)(node));
...
var getLastChild = (nodeEntry) => {
  const [node, path] = nodeEntry;
  if ((0, import_slate10.isText)(node)) return null;
...
var isLastChild = (parentEntry, childPath) => {
  ...
  return import_slate11.Path.equals(lastChildPath, childPath);
};

// src/queries/getLastNodeByLevel.ts
var import_slate12 = require("@udecode/slate");
var getLastChild2 = (node, level) => {
  if (!(level + 1) || !(0, import_slate12.isAncestor)(node)) return node;
  const { children } = node;
...
var getLastNodeByLevel = (editor, level) => {
  ...
  const lastNode = children.at(-1);
  if (!lastNode) return;
  const [, lastPath] = (0, import_slate12.getLastNode)(editor, []);
...
var getMark = (editor, key) => {
  if (!editor) return;
  const marks = (0, import_slate13.getMarks)(editor);
...
var getNextNodeStartPoint = (editor, at) => {
  const nextEntry = (0, import_slate14.getNextNode)(editor, {
    at
  });
  if (!nextEntry) return;
  return (0, import_slate14.getStartPoint)(editor, nextEntry[1]);
};
...
var getNodesRange = (editor, nodeEntries) => {
  ...
  return (0, import_slate15.getRange)(editor, firstBlockPath, lastBlockPath);
};
...
var isRangeAcrossBlocks = (editor, _a = {}) => {
  ...
  if (!at) return;
  const [start, end] = import_slate16.Range.edges(at);
  const startBlock = getBlockAbove(editor, __spreadValues({
    at: start
  }, options));
  const endBlock = getBlockAbove(editor, __spreadValues({
  ...
  return !import_slate16.Path.equals(startBlock[1], endBlock[1]);
};
...
var getPointBeforeLocation = (editor, at, options) => {
  ...
  if (!options || !options.match && !options.matchString) {
    return (0, import_slate17.getPointBefore)(editor, at, options);
  }
  ...
  matchStrings.some((matchString) => {
    var _a2, _b;
    let beforeAt = at;
    let previousBeforePoint = (0, import_slate17.getPoint)(editor, at, { edge: "end" });
    const stackLength = matchString.length + 1;
    const stack = Array.from({ length: stackLength });
    let count = 0;
    while (true) {
      const beforePoint = (0, import_slate17.getPointBefore)(editor, beforeAt, options);
      ...
      const beforeString = (0, import_slate17.getEditorString)(editor, {
        anchor: beforePoint,
        focus: previousBeforePoint
      });
...
var getPointFromLocation = (editor, {
  ...
  let point;
  if (import_slate18.Range.isRange(at)) point = focus ? at.focus : at.anchor;
  if (import_slate18.Point.isPoint(at)) point = at;
  if (import_slate18.Path.isPath(at)) point = { offset: 0, path: at };
  return point;
};
...
var getPointNextToVoid = (editor, {
  ...
  const startVoid = (0, import_slate19.getVoidNode)(editor, { at, mode: "highest" });
  if (startVoid) {
    const blockAbove = getBlockAbove(editor, { at });
    if (blockAbove) {
      const nextPoint = after ? (0, import_slate19.getPointAfter)(editor, at) : (0, import_slate19.getPointBefore)(editor, at);
      if (nextPoint && blockAbove && import_slate20.Path.isAncestor(blockAbove[1], nextPoint.path)) {
        at = nextPoint;
      }
    }
  }
...
var getPreviousBlockById = (editor, id, query) => {
  const entry = (0, import_slate21.findNode)(editor, {
    match: { id }
  });
  if (entry) {
    const prevEntry = (0, import_slate21.getPreviousNode)(editor, { at: entry[1] });
    if ((prevEntry == null ? void 0 : prevEntry[0].id) && (0, import_slate21.isBlock)(editor, prevEntry[0])) {
      return prevEntry;
    }
  }
  let found = false;
  const _nodes = (0, import_slate21.getNodeEntries)(editor, {
    at: [],
    match: (n) => {
      if (!(0, import_slate21.isBlock)(editor, n) || !n.id) return false;
      if (n.id === id) {
        found = true;
        return false;
      }
      return found && n.id !== id && (0, import_slate21.queryNode)([n, []], query);
    },
    mode: "highest",
    reverse: true
  });
  ...
  if (!found) return;
  const _entries = (0, import_slate21.getNodeEntries)(editor, {
    at: [],
    match: (n) => {
      return (0, import_slate21.isBlock)(editor, n) && !!n.id && (0, import_slate21.queryNode)([n, []], query);
    },
    mode: "highest"
  });
...
var getPreviousNodeEndPoint = (editor, at) => {
  const prevEntry = (0, import_slate22.getPreviousNode)(editor, {
    at
  });
  if (!prevEntry) return;
  return (0, import_slate22.getEndPoint)(editor, prevEntry[1]);
};
...
var getPreviousSiblingNode = (editor, path) => {
  ...
  if (index > 0) {
    ...
    const previousSiblingNode = (0, import_slate23.getNode)(editor, previousSiblingPath);
...
var getRangeBefore = (editor, at, options) => {
  ...
  if (!anchor) return;
  const focus = (0, import_slate24.getPoint)(editor, at, { edge: "end" });
  return {
    anchor,
    focus
  };
...
var getRangeFromBlockStart = (editor, options = {}) => {
  ...
  if (!path) return;
  const start = (0, import_slate25.getStartPoint)(editor, path);
...
var getSelectionFragment = (editor, options) => {
  ...
  const fragment = (0, import_slate26.getFragment)(editor, editor.selection);
...
var getSelectionText = (editor) => (0, import_slate27.getEditorString)(editor, editor.selection);

// src/queries/isAncestorEmpty.ts
var import_slate28 = require("@udecode/slate");
var isAncestorEmpty = (editor, node) => !(0, import_slate28.getNodeString)(node) && !node.children.some((n) => (0, import_slate28.isInline)(editor, n));
...
var isBlockTextEmptyAfterSelection = (editor) => {
  ...
  const selectionParentEntry = (0, import_slate29.getParentNode)(editor, editor.selection);
  if (!selectionParentEntry) return false;
  const [, selectionParentPath] = selectionParentEntry;
  if (!(0, import_slate29.isEndPoint)(editor, cursor, selectionParentPath)) return false;
  const siblingNodes = getNextSiblingNodes(blockAbove, cursor.path);
  if (siblingNodes.length > 0) {
    for (const siblingNode of siblingNodes) {
      if ((0, import_slate29.isText)(siblingNode) && siblingNode.text) {
        return false;
      }
    }
  } else {
    return (0, import_slate29.isEndPoint)(editor, cursor, blockAbove[1]);
  }
  return true;
};
...
var isDocumentEnd = (editor) => {
  if (editor.selection) {
    ...
    const endPoint = (0, import_slate30.getEndPoint)(editor, []);
    return endPoint.offset === 0 && (0, import_slate30.isEndPoint)(editor, point, point) && import_slate31.Path.equals(import_slate31.Path.next(import_slate31.Path.parent(point.path)), endPoint.path);
  }
  return false;
};
...
var isEditorEmpty = (editor) => {
  return editor.children.length === 1 && (0, import_slate32.isElementEmpty)(editor, editor.children[0]);
};
...
var isPointAtWordEnd = (editor, { at }) => {
  const after = (0, import_slate33.getPointAfter)(editor, at);
  const afterRange = (0, import_slate33.getRange)(editor, at, after);
  const afterText = (0, import_slate33.getEditorString)(editor, afterRange);
...
var isRangeInSameBlock = (editor, _a = {}) => {
  ...
  if (!at) at = editor.selection;
  if (!at) return;
  const [start, end] = import_slate34.Range.edges(at);
  const startBlock = getBlockAbove(editor, __spreadValues({
    at: start
  }, options));
  const endBlock = getBlockAbove(editor, __spreadValues({
  ...
  if (!startBlock || !endBlock) return;
  return import_slate34.Path.equals(startBlock[1], endBlock[1]);
};

// src/queries/isRangeInSingleText.ts
var import_slate35 = require("slate");
var isRangeInSingleText = (at) => {
  const [start, end] = import_slate35.Range.edges(at);
  return import_slate35.Path.equals(start.path, end.path);
};
...
var isSelectionAtBlockEnd = (editor, options) => {
  var _a, _b;
  const path = (_a = getBlockAbove(editor, options)) == null ? void 0 : _a[1];
  return !!path && (0, import_slate36.isEndPoint)(editor, (_b = editor.selection) == null ? void 0 : _b.focus, path);
};
...
var isSelectionAtBlockStart = (editor, options) => {
  ...
  return (0, import_slate37.isStartPoint)(editor, selection.focus, path) || (0, import_slate37.isExpanded)(editor.selection) && (0, import_slate37.isStartPoint)(editor, selection.anchor, path);
};
...
var isSelectionExpanded = (editor) => (0, import_slate38.isExpanded)(editor.selection);

// src/queries/isTextByPath.ts
var import_slate39 = require("@udecode/slate");
var isTextByPath = (editor, path) => {
  const node = (0, import_slate39.getNode)(editor, path);
  return (0, import_slate39.isText)(node);
};
...
var isWordAfterTrigger = (editor, { at, trigger }) => {
  const wordBefore = (0, import_slate40.getPointBefore)(editor, at, { unit: "word" });
  const before = wordBefore && (0, import_slate40.getPointBefore)(editor, wordBefore);
  const beforeRange = before && (0, import_slate40.getRange)(editor, before, at);
  const beforeText = (0, import_slate40.getEditorString)(editor, beforeRange);
...
var import_slate41 = require("@udecode/slate");
...
var queryEditor = (editor, {
  ...
  if (allows.length > 0 && !(0, import_slate41.someNode)(editor, { at, match: { type: allows } })) {
    return false;
  }
  const excludes = (0, import_castArray2.default)(exclude);
  if (excludes.length > 0 && (0, import_slate41.someNode)(editor, { at, match: { type: excludes } })) {
    return false;
  }
  return true;
};
...
var duplicateBlocks = (editor, blocks) => {
  ...
  editor.insertNodes(blocks.map((item) => item[0]), {
    at: import_slate42.Path.next(lastBlock[1])
  });
};
...
var insertElements = (editor, nodes, options) => {
  (0, import_slate43.insertNodes)(editor, nodes, options);
};
...
var insertEmptyElement = (editor, type, options) => {
  insertElements(
    ...
    (0, import_slate44.getQueryOptions)(editor, options)
  );
};
...
var moveChildren = (editor, { at, fromStartIndex = 0, match: match2, to }) => {
  let moved = 0;
  const parentPath = import_slate46.Path.isPath(at) ? at : at[1];
  const parentNode = import_slate46.Path.isPath(at) ? (0, import_slate45.getNode)(editor, parentPath) : at[0];
  if (!parentNode) return moved;
  if (!(0, import_slate45.isBlock)(editor, parentNode)) return moved;
  for (let i = parentNode.children.length - 1; i >= fromStartIndex; i--) {
    const childPath = [...parentPath, i];
    const childNode = (0, import_slate45.getNode)(editor, childPath);
    if (!match2 || childNode && match2([childNode, childPath])) {
      (0, import_slate45.moveNodes)(editor, { at: childPath, to });
      moved++;
    }
  }
  return moved;
...
var removeEditorText = (editor, _a = {}) => {
  ...
  (0, import_slate47.removeNodes)(editor, __spreadValues({
    at: [],
    match: (n, p) => {
      var _a2;
...
var removeEmptyPreviousBlock = (editor, options = {}) => {
  ...
  if (prevEntry && (0, import_slate48.isElementEmpty)(editor, prevEntry[0])) {
...
var removeMark = (editor, _a) => {
  ...
  if (selection) {
    if (import_slate50.Range.isRange(selection) && import_slate50.Range.isExpanded(selection)) {
      (0, import_slate49.unsetNodes)(editor, key, __spreadValues({
        at: selection,
        match: import_slate49.isText,
        split: true
      }, rest));
    } else if (editor.selection) {
      const marks = (_a2 = (0, import_slate49.getMarks)(editor)) != null ? _a2 : {};
      key.forEach((k) => {
        delete marks[k];
      });
      editor.marks = marks;
...
var removeNodeChildren = (editor, path, options) => {
  (0, import_slate51.withoutNormalizing)(editor, () => {
    for (const [, childPath] of (0, import_slate51.getNodeChildren)(editor, path, {
      reverse: true
    })) {
      (0, import_slate51.removeNodes)(editor, __spreadProps(__spreadValues({}, options), { at: childPath }));
    }
  });
};
...
var removeSelectionMark = (editor) => {
  const marks = (0, import_slate52.getMarks)(editor);
  if (!marks) return;
  Object.keys(marks).forEach((key) => {
    (0, import_slate52.removeEditorMark)(editor, key);
  });
};

// src/transforms/replaceNode.ts
var import_slate53 = require("@udecode/slate");
var replaceNode = (editor, { at, insertOptions, nodes, removeOptions }) => {
  (0, import_slate53.withoutNormalizing)(editor, () => {
    (0, import_slate53.removeNodes)(editor, __spreadProps(__spreadValues({}, removeOptions), { at }));
    (0, import_slate53.insertNodes)(editor, nodes, __spreadProps(__spreadValues({}, insertOptions), {
      at
    }));
  });
};
...
var import_slate54 = require("@udecode/slate");
var replaceNodeChildren = (editor, { at, insertOptions, nodes, removeOptions }) => {
  (0, import_slate54.withoutNormalizing)(editor, () => {
    removeNodeChildren(editor, at, removeOptions);
    (0, import_slate54.insertNodes)(editor, nodes, __spreadProps(__spreadValues({}, insertOptions), {
      at: at.concat([0])
    }));
  });
};
...
var import_slate55 = require("@udecode/slate");
var selectEndOfBlockAboveSelection = (editor) => {
  var _a;
  const path = (_a = getBlockAbove(editor)) == null ? void 0 : _a[1];
  path && (0, import_slate55.select)(editor, (0, import_slate55.getEndPoint)(editor, path));
};

// src/transforms/selectNodes.ts
var import_slate56 = require("@udecode/slate");
var selectNodes = (editor, nodes) => {
  const range = getNodesRange(editor, nodes);
  if (!range) return;
  (0, import_slate56.setSelection)(editor, range);
};
...
var import_slate59 = require("@udecode/slate");
var toggleWrapNodes = (editor, type) => {
  if ((0, import_slate59.someNode)(editor, { match: { type } })) {
    (0, import_slate59.unwrapNodes)(editor, { match: { type } });
  } else {
    (0, import_slate59.wrapNodes)(editor, {
      children: [],
      type
    });
  }
...
var import_slate60 = require("@udecode/slate");
var wrapNodeChildren = (editor, element, options) => {
  const path = options == null ? void 0 : options.at;
  const node = (0, import_slate60.getNode)(editor, path);
  if (!(node == null ? void 0 : node.children)) return;
  (0, import_slate60.withoutNormalizing)(editor, () => {
    const firstChildPath = path.concat([0]);
    (0, import_slate60.wrapNodes)(editor, element, __spreadProps(__spreadValues({}, options), {
      at: firstChildPath
    }));
