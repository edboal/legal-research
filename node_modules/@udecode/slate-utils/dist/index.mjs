...
var getBlocks = (editor, options) => {
  return [
    ...getNodeEntries(editor, __spreadProps(__spreadValues({}, options), {
      block: true
    }))
  ];
};
...
import { getNodeTexts as getNodeTexts2 } from "@udecode/slate";
function getFragmentProp(fragment, { key, defaultValue, getProp, mode = "block" } = {}) {
  ...
  for (const node of fragment) {
    ...
    if (mode === "text" || mode === "all") {
      const textEntries = Array.from(getNodeTexts2(node));
...
var getLastChild2 = (node, level) => {
  if (!(level + 1) || !isAncestor2(node)) return node;
  const { children } = node;
...
var getPointFromLocation = (editor, {
  ...
  let point;
  if (Range3.isRange(at)) point = focus ? at.focus : at.anchor;
  if (Point.isPoint(at)) point = at;
  if (Path4.isPath(at)) point = { offset: 0, path: at };
  return point;
};
...
var getPreviousBlockById = (editor, id, query) => {
  ...
  if (!found) return;
  const _entries = getNodeEntries2(editor, {
    at: [],
    match: (n) => {
...
import { getFragment } from "@udecode/slate";
...
var getSelectionFragment = (editor, options) => {
  ...
  const fragment = getFragment(editor, editor.selection);
...
var moveChildren = (editor, { at, fromStartIndex = 0, match: match2, to }) => {
  let moved = 0;
  const parentPath = Path10.isPath(at) ? at : at[1];
  const parentNode = Path10.isPath(at) ? getNode3(editor, parentPath) : at[0];
  if (!parentNode) return moved;
  if (!isBlock2(editor, parentNode)) return moved;
...
var removeEditorText = (editor, _a = {}) => {
  ...
  removeNodes(editor, __spreadValues({
    at: [],
    match: (n, p) => {
      var _a2;
...
import {
  isElementEmpty as isElementEmpty2
} from "@udecode/slate";
var removeEmptyPreviousBlock = (editor, options = {}) => {
  ...
  if (prevEntry && isElementEmpty2(editor, prevEntry[0])) {
...
var removeNodeChildren = (editor, path, options) => {
  withoutNormalizing(editor, () => {
    for (const [, childPath] of getNodeChildren(editor, path, {
      ...
      removeNodes2(editor, __spreadProps(__spreadValues({}, options), { at: childPath }));
    }
  });
};
...
var replaceNode = (editor, { at, insertOptions, nodes, removeOptions }) => {
  withoutNormalizing2(editor, () => {
    removeNodes3(editor, __spreadProps(__spreadValues({}, removeOptions), { at }));
